  ...
  doCommand(['config', 'providers'])
    .then(function (providers) {
      try {
        ...
        throw Error('Не удается прочитать список провайдеро��.')
      }
            ...
            const helpMessages = {
              ...
              'Skip Links': {
                Help: 'Не предупреждать о пропущенных символических ссылках.\n\n' +
                  'Этот флаг о��ключает сообщения предупреждения о пропущенных символических ссылках или точках соединения, так как вы явно признаете, что их следует пропустить.',
              ...
              'No Check Updated': {
                Help: 'Не проверять, изменяются ли файлы во время загрузки.\n\n' +
                  ...
                  'Если фа��л только дополняется (например, журнал), то rclone передаст файл журнала с размером, который он имел в первый раз, когда rclone его увидел.\n\n' +
              ...
              'No Clone': {
                Help: 'Отключить клонирование reflink для серверных копий.\n\n' +
                  ...
                  'В отличие от "жесткой ссы��ки", два файла независимы, и ни один из них не повлияет на другой, если они будут изменены впоследствии.\n\n' +
...
const UPDATE_DELAY = 1000; // 1 секунда задержки между обновлениями
...
const fireRcloneUpdateActions = function (eventName) {
  ...
  updateTimer = setTimeout(async () => {
    for (const callback of UpdateCallbacksRegistry) {
